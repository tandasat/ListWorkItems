...
EXTERN_C static KDEFERRED_ROUTINE ExclpRaiseIrqlAndWait;
...
_IRQL_raises_(DISPATCH_LEVEL) EXTERN_C void *ExclGainExclusivity() {
  ...
  auto context = reinterpret_cast<ExclusivityContext *>(ExAllocatePoolWithTag(
      NonPagedPool, sizeof(void *) + (numberOfProcessors * sizeof(KDPC)),
      EXCLP_POOL_TAG));
  if (!context) {
    return nullptr;
  }
...
EXTERN_C static void ExclpRaiseIrqlAndWait(__in PKDPC Dpc,
